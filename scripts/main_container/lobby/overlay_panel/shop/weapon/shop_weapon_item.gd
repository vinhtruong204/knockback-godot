...
func set_shop_item(item: Dictionary, owned_ids: Dictionary = {}) -> void:
	...
	weapon_texture.mouse_filter = Control.MOUSE_FILTER_STOP
	weapon_texture.gui_input.connect(_on_texture_input)
	...
	PlayerApi.purchase_item(
		...
		func(response: Dictionary) -> void:
			if response.get("ok", false):
				...
				# Mark as owned locally so the shop tile reflects the purchase
				# without waiting for a re-fetch. Stays disabled.
				_is_owned = true
				buy_button.text = "Owned"
