...
func _ready() -> void:
	...
	mode_button.text = "Free for all"
	team_size_button.text = "1v1"
...
func _on_start_button_pressed() -> void:
	...
	if not port_text.is_valid_int():
		_show_error("Invalid port: not a number")
		return

	var port = port_text.to_int()
	if port < 1024 or port > 65535:
		_show_error("Invalid port: must be between 1024 and 65535")
		return
...
func _update_map_button() -> void:
	map_button.text = MAPS[_selected_map_index]
