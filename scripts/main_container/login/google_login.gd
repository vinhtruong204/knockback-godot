...
func _ready():
	...
	if ApiManager.is_logged_in():
		text_edit.text = "Verifying session..."
...
func _on_session_verify(response: Dictionary) -> void:
	...
	if status == 401 or status == 403:
		...
		text_edit.text = "Session expired. Please sign in again."
	else:
		...
		text_edit.text = "Could not verify session: " + str(response.get("error", "")) + ". Please try again."
